...
import kanierp
from kanierp.setup.default_energy_point_rules import get_default_energy_point_rules
from kanierp.setup.doctype.incoterm.incoterm import create_incoterms

from .default_success_action import get_default_success_action

default_mail_footer = """<div style="padding: 7px; text-align: right; color: #888"><small>Sent via
	<a style="color: #888" href="http://frappe.io/kanierp">kanierp</a></div>"""
...
def check_setup_wizard_not_completed():
	if cint(frappe.db.get_single_value("System Settings", "setup_complete") or 0):
		message = """kanierp can only be installed on a fresh site where the setup wizard is not completed.
...
def check_frappe_version():
	...
	kanierp_version = major_version(kanierp.__version__)

	if frappe_version == kanierp_version:
		return

	click.secho(
		f"You're attempting to install kanierp version {kanierp_version} with Kanivin version {frappe_version}. "
...
def add_standard_navbar_items():
	...
	kanierp_navbar_items = [
		{
			"item_label": "Documentation",
			"item_type": "Route",
			"route": "https://docs.kanierp.com/",
			"is_standard": 1,
		},
		{
	...
	for item in kanierp_navbar_items:
...
def add_app_name():
	frappe.db.set_single_value("System Settings", "app_name", "kanierp")
